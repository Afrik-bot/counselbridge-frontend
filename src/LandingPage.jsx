...
const css = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body { background: var(--navy); color: #fff; font-family: var(--sans); overflow-x: hidden; line-height: 1.6; }
  ...
  .nav-logo img { height: 48px; width: auto; object-fit: contain; transition: opacity 0.2s, height 0.3s; }
  ...
  .nav-cta { background: #2563EB; color: white !important; padding: 9px 22px; border-radius: 8px; font-weight: 600 !important; }
  .nav-cta:hover { background: #3B82F6 !important; transform: translateY(-1px); }
  ...
  .hero-badge-dot { width: 6px; height: 6px; background: #3B82F6; border-radius: 50%; animation: blink 2s infinite; }

  @keyframes blink { 0%,100%{opacity:1;} 50%{opacity:0.4;} }

  .hero-title {
    font-family: var(--serif); font-size: clamp(52px, 7vw, 88px); font-weight: 300;
    line-height: 1.05; letter-spacing: -0.03em; margin-bottom: 28px;
    animation: fadeUp 0.8s ease 0.1s both;
  }
  .hero-title em { font-style: italic; color: #E2C47A; }
  ...
  .hero-actions { display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; animation: fadeUp 0.8s ease 0.3s both; }

  .btn-primary {
    background: #2563EB; color: white; padding: 14px 32px; border-radius: 10px;
    font-size: 15px; font-weight: 600; text-decoration: none; display: inline-flex;
    align-items: center; gap: 8px; transition: all 0.2s; border: none; cursor: pointer;
    font-family: var(--sans); box-shadow: 0 0 40px rgba(37,99,235,0.3);
  }
  .btn-primary:hover { background: #3B82F6; transform: translateY(-2px); }

  .btn-ghost {
    background: rgba(255,255,255,0.06); color: rgba(255,255,255,0.8); padding: 14px 32px;
  ...
  .hero-stat-value { font-family: var(--serif); font-size: 36px; font-weight: 600; letter-spacing: -0.03em; }
  ...
  .section-label { font-size: 12px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #3B82F6; margin-bottom: 16px; display: flex; align-items: center; gap: 10px; }
  .section-label::before { content: ''; display: inline-block; width: 24px; height: 1px; background: #3B82F6; }
  ...
  .section-title em { font-style: italic; color: #E2C47A; }
  ...
  .features-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 2px; background: rgba(255,255,255,0.04); border-radius: 20px; overflow: hidden; border: 1px solid rgba(255,255,255,0.06); }
  .feature-card { background: #0A1628; padding: 36px 32px; transition: background 0.3s; position: relative; overflow: hidden; }
  ...
  .feature-title { font-family: var(--serif); font-size: 20px; font-weight: 600; margin-bottom: 10px; }
  ...
  .ai-inner {
    ...
    background: linear-gradient(135deg, #0F2240 0%, rgba(37,99,235,0.08) 100%);
    border-radius: 28px; border: 1px solid rgba(37,99,235,0.2);
    padding: 72px; display: grid; grid-template-columns: 1fr 1fr; gap: 72px;
  ...
  .pricing-card { background: #0A1628; border: 1px solid rgba(255,255,255,0.07); border-radius: 20px; padding: 36px 32px; position: relative; transition: transform 0.3s, border-color 0.3s; }
  .pricing-card:hover { transform: translateY(-4px); border-color: rgba(37,99,235,0.3); }
  .pricing-card.featured { background: linear-gradient(135deg, #0F2240 0%, rgba(37,99,235,0.1) 100%); border-color: rgba(37,99,235,0.4); box-shadow: 0 0 60px rgba(37,99,235,0.12); }
  .pricing-badge { position: absolute; top: -12px; left: 50%; transform: translateX(-50%); background: #2563EB; color: white; font-size: 11px; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; padding: 4px 14px; border-radius: 100px; white-space: nowrap; }
  ...
  .pricing-cta-primary { background: #2563EB; color: white; border: none; }
  .pricing-cta-primary:hover { background: #3B82F6; transform: translateY(-1px); }
  .pricing-cta-ghost { background: transparent; color: rgba(255,255,255,0.7); border: 1px solid rgba(255,255,255,0.12); }
  .pricing-cta-ghost:hover { background: rgba(255,255,255,0.06); }
      ...
      <section className="ai-section" id="ai">
        <div className="ai-inner fade-up">
          ...
          <div style={{ position: "relative", zIndex: 2 }}>
            ...
            {[
              { label: "Zone 1 — Automated", desc: "Informational only", color: "#059669", dot: "#34D399" },
              { label: "Zone 2 — Attorney Review", desc: "Requires your approval", color: "#2563EB", dot: "#60A5FA" },
              { label: "Zone 3 — Human Only", desc: "Legal advice & strategy", color: "#C9A84C", dot: "#E2C47A" },
            ].map((z) => (
              <div key={z.label} className="ai-zone" style={{ borderColor: z.color + "33", background: z.color + "12" }}>
